...
primeList = primes.upto(104729) #10 000 primes
...
class Device:
    ...
    def registerDevice(self,name:str,server):
        factors = primes.factors(self.DK)

        newDID = None
        while newDID is not None:
...
class Server:
    ...
    def __init__(self,DB1,DB2,DB3,p=29996224275833):
        self.DB1 = DB1
        self.DB2 = DB2
        self.DB3 = DB3
        self.p = p
    ...
    def deviceRevocation(self,Device,message,DIDtoRevoke=None):
        existingDIDs = [DID for (UID,DID),DSK in DB1.items() if UID == Device.UID and DSK != None]
